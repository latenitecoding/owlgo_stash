...
pub fn main() !void {
    ...
    var r_need = try r.next(i16);
    var g_need = try r.next(i16);
    var b_need = try r.last(i16);

    const r_have = try r.next(i16);
    const g_have = try r.next(i16);
    const b_have = try r.last(i16);

    var rg_avail = try r.next(i16);
    var gb_avail = try r.last(i16);
